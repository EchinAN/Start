...
func main() {
	...
	for i := 1; i <= 5; i++ {
		...
		OtvetP, err := http.Get("https://api.vk.com/method/wall.get?owner_id=-41774259&count=10&offset=0&access_token=13d3de0213d3de0213d3de029b13b3c292113d313d3de0249e43129fa9241a0bc1fa3e5&v=5.52")
		if err != nil {
			panic(err)
		}
		...
		for _, rt := range UnmarshP.Response.Items{
			if rt.Date > dataPost[len(dataPost)-1] {
				fmt.Println("Текст поста", rt.Text, "Текст поста")
		...
		for _, row := range UnmarshP.Response.Items {
			...
			for _, rowC := range UnmarshC.Response.Items { //Текс комментариев
				if rowC.Date > dataComments[len(dataComments)-1]{
					fmt.Println("Текст комментария",rowC.Text, "Текс комментария")
					dataComments = append(dataComments, rowC.Date)
				}else {
					continue
				}
